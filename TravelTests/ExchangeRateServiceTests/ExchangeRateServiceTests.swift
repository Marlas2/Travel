...
class ExchangeRateServiceTests: XCTestCase {
    func testGetRateShouldPostFailedCallbackIfError() {
        ...
        exchangeService.getRate { (success, rate) in
            // Then
    ...
    func testGetRateShouldPostFailedCallbackIfIncorrectData() {
        // Given
       let exchangeService = ExchangeRateService(rateSession: URLSessionFake(data: FakeResponseData.exchangeIncorrectData, response: FakeResponseData.responseOK, error: FakeResponseData.error))
        // When
        let expectation = XCTestExpectation(description: "Wait for queue change.")
        exchangeService.getRate { (success, rate) in
            // Then
    ...
    func testGetRateShouldPostSuccessCallbackIfCallbackFalseNil() {
        ...
        let exchangeService = ExchangeRateService(rateSession: URLSessionFake(data: FakeResponseData.exchangeIncorrectData, response: FakeResponseData.responseOK, error: nil))
        // When
        let expectation = XCTestExpectation(description: "Wait for queue change.")
        exchangeService.getRate { (success, rate) in
            // Then
