...
class WeatherTestCase: XCTestCase {
    ...
    func testGetWeatherShouldPostFailedCallbackIfIncorrectData() {
        // Given
        let weather = WeatherService(weatherSession: URLSessionFake(data: FakeResponseData.exchangeIncorrectData, response: FakeResponseData.responseOK, error: FakeResponseData.error))
        // When
        let expectation = XCTestExpectation(description: "Wait for queue change.")
        weather.getWeather { (success, weather) in
            // Then
    ...
    func testGetWeatherShouldPostSuccessCallbackIfNoErrorAndCorrectData() {
        // Given
        let weather = WeatherService(weatherSession: URLSessionFake(data: FakeResponseData.exchangeCorrectData, response: FakeResponseData.responseOK, error: nil))
        // When
        let expectation = XCTestExpectation(description: "Wait for queue change.")
        weather.getWeather { (success, weather) in
            // Then
    ...
    func testGetWeatherShouldPostSuccessCallbackIfCallbackFalseNil() {
        ...
        let weather = WeatherService(weatherSession: URLSessionFake(data: FakeResponseData.exchangeIncorrectData, response: FakeResponseData.responseOK, error: nil))
        // When
        let expectation = XCTestExpectation(description: "Wait for queue change.")
        weather.getWeather { (success, weather) in
            // Then
